...
public:
	...
	DelayExpSynth(){
		...
		ControlRandom rand = ControlRandom().min(0).max(addParameter("rand_max", 36.f).min(1.0f).max(72.f)).trigger(metro); //spi
		...
		ControlMidiToFreq freq = ControlMidiToFreq().input(addParameter("base_freq", 36.f).min(1.0f).max(72.f) + snap); //spi

		ControlParameter pulsewidth = addParameter("pulsewidth", 0.5f).min(0.050f).max(1.0f); //spi

		Generator osc = (
			(
			(
			//RectWave().freq(freq * 0.99).pwm(0.5f) * aEnv * 0.5
			RectWave().freq(freq * 0.99).pwm(pulsewidth) * aEnv * 0.5
			)
			...
			RectWave().freq(freq * 1.01).pwm(pulsewidth) * aEnv * 0.5
			)
			>> MonoToStereoPanner().pan(0.5)
			)
			); //original
		...
		*/ //spi
		...
		ControlParameter lpf_cutoff = addParameter("lpf_cutoff", 400.0f).min(50).max(10000); //spi
		ControlParameter lpf_q = addParameter("lpf_q", 1.1f).min(0.1).max(100); //spi
